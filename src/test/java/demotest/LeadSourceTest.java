...
public class LeadSourceTest extends base {
   ...
   //adminmenu
		WebElement adminMenu = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[normalize-space(text())='Admin']")));
		adminMenu.click();
		...
		// Click on Lead Settings
				WebElement leadSettings = wait.until(
						ExpectedConditions.elementToBeClickable(By.xpath("//ul[@id='menu']//a[contains(normalize-space(.), 'Lead settings')]")));
				System.out.println("lead settings located");
				leadSettings.click();
		...
		WebElement leadRequirement = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//a[normalize-space(text())='Lead Requirement']")));
	...
	public void createLeadRequirement() {
		...
		WebElement createBtn = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("//*[@id='lead_requirement']/div/div/div[1]/span/button")));
		createBtn.click();

		// ✅ Enter Lead Requirement name
		String requirementName = "Test Requirement " + System.currentTimeMillis() % 100000;
		WebElement reqInput = wait.until(ExpectedConditions.visibilityOfElementLocated(By.name("name") // <-- change if
																										// the input
																										// field has a
																										// different
																										// 'name'
		...
		WebElement createdRequirement = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(
				"//*[@id='lead_requirement']//li[contains(@class,'list-group-item') and normalize-space(text())='"
						+ requirementName + "']")));
	...
	public void editLeadRequirement() {
		...
		WebElement editIcon = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("(//div[@id='lead_requirement']//li[1]//span[1]//*[name()='svg'])[1]")));
		editIcon.click();
		System.out.println("Edit icon clicked...");

		// ✅ Edit name input field (relative xpath for input inside lead_requirement)
		WebElement requirementInput = wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//div[@id='lead_requirement']//input[@type='text']")));
		...
		// ✅ Validate update
		WebElement updatedRequirementElement = wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//li[normalize-space(text())='" + updatedRequirement + "']")));
	...
	public void deleteLeadRequirement() {
		...
		WebElement deleteIcon = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("//div[@id='lead_requirement']//ul/li[1]//span[2]//svg")));
		deleteIcon.click();
		System.out.println("Delete icon clicked...");

		// ✅ Confirm delete in popup
		WebElement deleteConfirm = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(
		...
		WebElement okButton = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(@class,'swal-button--confirm')]")));
		okButton.click();
